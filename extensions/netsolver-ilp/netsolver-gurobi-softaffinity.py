...
parser.add_argument("VirtualNetwork", help="Path to the virtual network file")
...
parser.add_argument("OutputFile", help="File to stream timestamp output to after each allocation is made")
...
#m.write("file.lp")
affinityConstrs = vn["affinity"]
affinityVarIndices = tuplelist([(i, j) for i in range(0, len(affinityConstrs)) for j in pn["Servers"].keys()])
affinityVars = m.addVars(affinityVarIndices, name="affinityVars", vtype=GRB.BINARY)
modelAffinityConstrs = {}
for i in pn["Servers"].keys():
    for j in range(0, len(affinityConstrs)):
        listPlaceVars = [place[k, i] for k in affinityConstrs[j]]
        modelAffinityConstrs[j, i] = m.addConstr(affinityVars[(j, i)] == max_(listPlaceVars), "modelAffinityConstrs[" + str(j) + "," + i + "]")

for i in range(0, len(affinityConstrs)):
    m.setObjectiveN(affinityVars.sum(i, '*'), i)
